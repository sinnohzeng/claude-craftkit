...
轩匠是一个专注中文写作的 Claude Code 插件。它把任仲然 40 年公文写作方法论（《怎样写作》）转化为 AI 可执行的工作流，并配套了一套 140 余条规则的中文写作规范检查机制。规范来源覆盖 GB/T 15834-2011《标点符号用法》、GB/T 9704-2012《党政机关公文格式》、Wikipedia 官方《Signs of AI writing》以及国务院办公厅、国家信息中心、国务院发展研究中心、财政部等机关的真实文件参考。
...
writing-polish v4.1 配套了三组方法对应这三种情况。
...
三是建立 140 余条规则与 `scripts/scan-ai-taste.sh` 自动校验。AI 改完的稿子必须通过 grep 扫描才允许交付。
...
├── 标点格式  14 条（含 v4.1 新增 §1.4.111 至 §1.4.113）
└── 戏剧化偏好（§1.5，v4.1 新增）
    ├── §1.5.1 战斗化叙事
    ├── §1.5.2 互联网大厂黑话
    ├── §1.5.3 网络口语与网感词
...
├── SKILL.md                          主入口（v4.0 从 446 行压缩到约 220 行）
├── scripts/
│   ├── scan-ai-taste.sh              第三步自动校验
│   ├── check-cn-quotes.py            标点与中英混排校验（弯引号、加号、直角引号、缝合词）
│   └── word-count-check.sh           句长方差与段落同质化检查
├── references/                       按需载入的 8 个深度文档
│   ├── anti-ai-taste-anchors.md      124 条红线、60 条橙线、15 条结构反模式
│   ├── ai-taste-examples.md          10 段反例对照
...
├── assets/                           用于产出的素材
│   ├── anchor-essays/                《怎样写作》原书 8 范文
│   ├── real-world-anchors/           5 个真实政府文件参考
...
| v4.0.0 | 2026-04 | 110 条 AI 味规则、8 范文、三步检查、符合 2026 Anthropic Skills 标准 |
| v4.1.0 | 2026-04 | 新增戏剧化偏好分类（§1.5），新增 GB/T 15834 弯引号、数学符号、半中半英三类红线，规则总数到 140 余条 |
