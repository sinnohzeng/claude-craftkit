...
2. **是否需要回写 DOCX？**
   - 需要回写，先阅读 `references/docx-editing-guide.md` 进入 DOCX 编辑工作流
...
启动写作或审稿任务前，必读：
1. `references/anti-ai-taste-anchors.md` 的 §0 核心机制和 §1 红线 110 条
2. `references/ai-taste-examples.md` 的 10 段反例对照（让眼睛记住什么是 AI 味）
3. 对应文体在 `assets/anchor-essays/` 中的 1 至 2 个范文，以及 `assets/real-world-anchors/` 中的真实文件参考
...
| **`anti-ai-taste-anchors.md`** | **110 条红线、60 条橙线、15 条结构反模式以及量化阈值** | **任何写作或修改前必读** |
| **`ai-taste-examples.md`** | **10 段反例对照** | **首次使用此技能时必读** |

### assets（用于产出的素材）

| 目录 | 内容 | 何时用 |
|---|---|---|
| `anchor-essays/` | 《怎样写作》原书的 8 个范文 | 写作辅助时摹仿 |
| `real-world-anchors/` | 5 个真实文件参考，来源含国办、国家信息中心、国务院发展研究中心、财政部 | 公文或咨询报告对标 |
